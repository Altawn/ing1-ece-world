#include "head.h"

void ecran(){
    BITMAP* page;
    page = load_bitmap("../assets/images/presentation/pokepark.bmp",NULL);
    while(!key[KEY_ENTER]){
        blit(page,screen,0,0,0,0,SCREEN_W,SCREEN_H);

        textout_centre_ex(screen, font, "press enter to continue",512,445, makecol(0, 0, 0), -1);
        rest(100);

    }
}

void print_chen(BITMAP *page,t_player* player,t_player* player2) {

    char nom_perso1[256] = "";
    char stock_nom1[256];
    char nom_perso2[256] = "";
    char stock_nom2[256];
    int position_lettre = 0;
    int position_lettre2 = 0;

    bool draw = true;
    bool swap1 = false;
    bool swap2 = false;
    bool swap3 = false;
    bool swap4 = false;
    bool swap5 = false;
    bool swap6 = false;

    bool circle = false;
    int pos = 257;
    int i = 0;
    int yrec =0;
    int xrec=0;
    char* texte[] = {"Bonjour, je suis  le Professeur Chene.","Votre honneur se porte-il bien ?","Puis-je vous demander comment on vous prenomme, s'il vous plait ?","Et comment votre coéquipier est il prénommé, je vous prie ?","Quelle apparence souhaiteriez-vous adopter ?","Et celle de l'autre individu, je vous prie ?","Vous allez penetrer dans le parc d'attractions Pokemon de l'ECE", "Etes-vous prêts ?","C'est bien dommage","d"};
    bool text = true;
    bool flag = false;
    BITMAP *chen;BITMAP* lucas;BITMAP* ethan;
    ethan = load_bitmap("../assets/images/presentation/ethan.bmp",NULL);
    lucas = load_bitmap("../assets/images/presentation/lucas.bmp",NULL);
    chen = load_bitmap("../assets/images/presentation/pr_chen1.bmp", NULL);
    int current_phrase = 0;
    char display[256];
    memset(display, 0, sizeof(display)); // Initialisation de la chaîne de caractères à afficher


    while (!flag) {

        if (text) {
            if (i < strlen(texte[current_phrase]) && current_phrase <=8) {
                display[i] = texte[current_phrase][i];
                i++;
            }

            else {
                rest(500);
                memset(display, 0, sizeof(display));
                i = 0;
                if(current_phrase <= 8){
                    current_phrase = current_phrase + 1;
                }
            }

            ///////////BULLE 1///////

            if(current_phrase == 2 && swap1 == false ){
                swap1 = true;
                while (!key[KEY_ENTER]) {
                    circle = false;
                    if(key[KEY_UP]){
                        pos = 257;
                        circle = true;
                    }
                    if(key[KEY_DOWN]){
                        circle = true;
                        pos = 277;
                    }
                    if ( xrec<100){
                        rectfill(screen,237,239,(247+xrec),(249+yrec), 0);
                        rectfill(screen,242,244,(242+xrec),(244+yrec), makecol(255,255,255));
                        rest(10);
                        yrec++;
                        xrec= xrec+2;
                    }
                    if(xrec ==100){
                        if(circle == true){
                            rectfill(screen,242,244,(242+xrec),(244+yrec), makecol(255,255,255));
                        }
                        textprintf_ex(screen,font,262,254,0,-1,"OUI");
                        textprintf_ex(screen,font,262,274,0,-1,"NON");
                        circlefill(screen,252,pos,2,0);
                    }
                }
            }

            ///////// BULLE 2///////
            if(current_phrase == 3 && swap2 == false ){
                swap2 = true;
                while (!key[KEY_ENTER]) {
                    if ( xrec<150){
                        rectfill(screen,637,229,(647+xrec),(239+yrec), 0);
                        rectfill(screen,642,234,(642+xrec),(234+yrec), makecol(255,255,255));
                        rest(10);
                        yrec++;
                        xrec= xrec+6;
                    }
                    if (keypressed()) {
                        int key_code = readkey();
                        char lettre_nom = key_code & 0xFF;
                        if (lettre_nom >= ' ' && lettre_nom <= '~' && position_lettre < 255) {
                            nom_perso1[position_lettre++] = lettre_nom;
                            nom_perso1[position_lettre] = '\0';
                        }
                        if(key[KEY_BACKSPACE]){
                            if(position_lettre > 0){
                                for (int j = position_lettre; j != -1; j--) {
                                    nom_perso1[j] = '\0';
                                }
                                rectfill(screen,642,234,(632+xrec),(224+yrec), makecol(255,255,255));
                                position_lettre=0;
                            }
                        }
                    }
                    textout_ex(screen, font, nom_perso1, 652, 244, makecol(0, 0, 0), -1);
                }
            }

            ////////BULLE 2 BIS///////
            if(current_phrase == 4 && swap3 == false ){
                swap3 = true;
                while (!key[KEY_ENTER]) {
                    if (draw){
                        rectfill(screen,637,229,(647+xrec),(239+yrec), 0);
                        rectfill(screen,642,234,(642+xrec),(234+yrec), makecol(255,255,255));
                        draw = false;
                    }
                    if (keypressed()) {
                        int key_code = readkey();
                        char lettre_nom = key_code & 0xFF;
                        if (lettre_nom >= ' ' && lettre_nom <= '~' && position_lettre < 255) {
                            nom_perso2[position_lettre2++] = lettre_nom;
                            nom_perso2[position_lettre2] = '\0';
                        }
                        if(key[KEY_BACKSPACE]){
                            if(position_lettre2 > 0){
                                for (int j = position_lettre2; j != -1; j--) {
                                    nom_perso2[j] = '\0';
                                }
                                rectfill(screen,642,234,(632+xrec),(224+yrec), makecol(255,255,255));
                                position_lettre2=0;
                            }
                        }
                    }
                    textout_ex(screen, font, nom_perso2, 652, 244, makecol(0, 0, 0), -1);
                }
            }
            if(current_phrase == 5 && !swap5 ){
                swap5 = true;
                int pos_selecx = 635;
                while (!key[KEY_ENTER]){
                    clear_to_color(page, makecol(82,107,231));
                    blit(chen, page, 0, 0, 192, 144, SCREEN_W, SCREEN_H);
                    if(key[KEY_RIGHT]){
                        pos_selecx = 635;
                    }
                    if(key[KEY_LEFT]){
                        pos_selecx = 285;
                    }

                    rectfill(page,625,275,735,435,0);
                    rectfill(page,275,275,385,435,0);
                    rectfill(page,630,280,730,430, makecol(255,255,255));
                    rectfill(page,280,280,380,430, makecol(255,255,255));
                    rect(page,pos_selecx,285,pos_selecx+90,425, makecol(255,0,0));
                    draw_sprite(page,lucas,650,300);
                    draw_sprite(page,ethan,300,300);
                    blit(page,screen,0,0,0,0,SCREEN_W,SCREEN_H);
                    if(key[KEY_ENTER] && pos_selecx == 285){
                        player->type = 1;
                    }
                    if(key[KEY_ENTER] && pos_selecx == 635){
                        player->type = 2;
                    }

                }
            }
            if(current_phrase == 6 && !swap6 ){
                swap6 = true;
                int pos_selecx = 635;
                while (!key[KEY_ENTER]){
                    clear_to_color(page, makecol(82,107,231));
                    blit(chen, page, 0, 0, 192, 144, SCREEN_W, SCREEN_H);
                    if(key[KEY_RIGHT]){
                        pos_selecx = 635;
                    }
                    if(key[KEY_LEFT]){
                        pos_selecx = 285;
                    }

                    rectfill(page,625,275,735,435,0);
                    rectfill(page,275,275,385,435,0);
                    rectfill(page,630,280,730,430, makecol(255,255,255));
                    rectfill(page,280,280,380,430, makecol(255,255,255));
                    rect(page,pos_selecx,285,pos_selecx+90,425, makecol(255,0,0));
                    draw_sprite(page,lucas,650,300);
                    draw_sprite(page,ethan,300,300);
                    blit(page,screen,0,0,0,0,SCREEN_W,SCREEN_H);
                    if(key[KEY_ENTER] && pos_selecx == 285){
                        player2->type = 1;
                    }
                    if(key[KEY_ENTER] && pos_selecx == 635){
                        player2->type = 2;
                    }

                }
            }

            ////BULLE 3/////
            if(current_phrase == 8 && swap4 == false ){
                swap4 = true;
                xrec = 0;
                yrec =0;
                while (!key[KEY_ENTER]) {
                    circle = false;
                    if(key[KEY_UP]){
                        pos = 257;
                        circle = true;
                    }
                    if(key[KEY_DOWN]){
                        circle = true;
                        pos = 277;
                    }
                    if ( xrec<100){
                        rectfill(screen,237,239,(247+xrec),(249+yrec), 0);
                        rectfill(screen,242,244,(242+xrec),(244+yrec), makecol(255,255,255));
                        rest(10);
                        yrec++;
                        xrec= xrec+2;
                    }
                    if(xrec ==100){
                        if(circle == true){
                            rectfill(screen,242,244,(242+xrec),(244+yrec), makecol(255,255,255));
                        }
                        textprintf_ex(screen,font,262,254,0,-1,"OUI");
                        textprintf_ex(screen,font,262,274,0,-1,"NON");
                        circlefill(screen,252,pos,2,0);
                    }
                }

                if(pos == 257){
                    flag = true;
                }

            }
            if(current_phrase == 9){
                while(key[KEY_ENTER]){
                    rest(1);
                }
                allegro_exit();
            }

            clear_to_color(page, makecol(82,107,231));
            blit(chen, page, 0, 0, 192, 144, SCREEN_W, SCREEN_H);
            blit(page, screen, 0, 0, 0, 0, SCREEN_W, SCREEN_H);
            textout_ex(screen, font, display, 232, 544, makecol(0, 0, 0), -1);


            rest(25);

        }

    }
    strcpy(player->name,nom_perso1);
    strcpy(player2->name,nom_perso2);
}